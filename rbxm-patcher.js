...
function parseFile(buf){
  ...
  while(true){
    ...
    chunks.push({ name: hdr.name, data });
...
function serializeChunk(name, data){
  const compressed = compressBlock(data);
  const useCompression = compressed.length > 0 && compressed.length < data.length;
  ...
  if(useCompression){
    ...
    out.set(header,0); out.set(compressed,16);
    return out;
  } else {
...
function rebuildFile(parsed, chunkOverrides){
  ...
  parsed.chunks.forEach((chunk, i) => {
    const data = chunkOverrides.has(i) ? chunkOverrides.get(i) : chunk.data;
    parts.push(serializeChunk(chunk.name, data));
  });
